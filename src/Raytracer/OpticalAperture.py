...
class OpticalAperture(object):
    ...
    def generateEdge(self):
        self.edge = np.array([np.array([]), np.array([]), np.array([]), np.array([])])
...
class CircularAperture(OpticalAperture):
    ...
    def generateEdge(self):
        ...
        self.edge = np.vstack((self.size*np.cos(theta), self.size*np.sin(theta), np.zeros(nbrPoints), np.ones(nbrPoints)))
