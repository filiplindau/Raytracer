...
lens = oe.PCXElement(x=np.array([-136e-3,0,200e-3]), n=1.5, r=0.1, thickness=5e-3, material = optSys.materialLibrary.getMaterial('fs'), size=12.7e-3)
lens.rotateElement(0, 47*np.pi/180)
lens2 = oe.PCXElement(x=np.array([-409e-3,0,500e-3]), n=1.5, r=0.1, thickness=5e-3, material = optSys.materialLibrary.getMaterial('fs'))
lens2.flipElement()
...
screen = oe.ScreenElement(x=np.array([-850e-3,0,725e-3]))
screen.rotateElement(0, 90*np.pi/180)

r1 = rs.Collimated1DSource(numRays=20, xDim=10e-3, l=263e-9, color=(0.35,0,0.75))
